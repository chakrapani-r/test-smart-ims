...
async def update_book(request: RequestBook, db: Session = Depends(get_db)):
    ...
    return Response(code="200", status="ok", message="Successfully updated the book", result=_book).dict(exclude_none=True)


@router.delete('/{id}')
async def delete(id: int, db: Session = Depends(get_db)):
    crud.remove_book(db=db, book_id=id)
    return Response(code="200", status="ok", message="Successfully deleted the book")
